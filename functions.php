...
function themeConfig($form) {
    ...
    $bg = new Typecho_Widget_Helper_Form_Element_Text('bg', NULL, NULL, _t('导航栏'), _t('导航栏背景'));
    $form->addInput($bg);
    
    $motto = new Typecho_Widget_Helper_Form_Element_Text('motto', NULL, NULL, _t('简介'), _t('显示在顶部'));
    $form->addInput($motto);

    $bs = new Typecho_Widget_Helper_Form_Element_Text('bs', NULL, NULL, _t('建站日期'), _t('显示在底部'));
    $form->addInput($bs);
...
{ 
    // 当文章无图片时的默认缩略图
    $dir = './usr/themes/default/img/sj/';//随机缩略图目录
...
foreach ($widget->tags as $tag) {
    ...
    $ctu = './usr/themes/default/img/tag/' . $tag['slug'] . '.jpg';
